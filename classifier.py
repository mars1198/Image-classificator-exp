...
images_dir_train = 'genus'
images_dir_test = 'genus64'
...
def plot_features_train(feature_labels, t_sne_features):
    
    plt.figure(figsize=(9, 9), dpi=100)

    uniques = {x: labels_train.count(x) for x in feature_labels}
...
def plot_features_test(feature_labels, t_sne_features):
    
    plt.figure(figsize=(9, 9), dpi=100)

    uniques = {x: labels_test.count(x) for x in feature_labels}
    od = collections.OrderedDict(sorted(uniques.items()))
...
dir_list = [x[0] for x in os.walk(images_dir_train)]
dir_list = dir_list[1:]
list_images = []
...
features_train, labels_train = extract_features(list_images)



dir_list = [x[0] for x in os.walk(images_dir_test)]
dir_list = dir_list[1:]
list_images = []
for image_sub_dir in dir_list:
	sub_dir_images = [image_sub_dir + '/' + f for f in os.listdir(image_sub_dir) if re.search('jpg|JPG', f)]
...
features_test, labels_test = extract_features(list_images)
...
tsne_features_train = TSNE().fit_transform(features_train)
tsne_features_test = TSNE().fit_transform(features_test)


plot_features_train(labels_train, tsne_features_train)
plot_features_test(labels_test, tsne_features_test)
